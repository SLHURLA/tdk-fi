...
export async function GET(
  ...
  try {
    ...
    // Total expenses
    totalExpenses = storeExpenses.reduce(
      (sum, exp) => sum + (exp.amount || 0),
      0,
    );

    // ── Leads loop: time-based aggregations ──────────────────────────────────
    // These leads are already filtered to status: "CLOSED", so bucket them by
    // handoverDate (when they actually closed) — matching how the Revenue
    // rollup (projectClose/revenue below) is bucketed, not by createdAt.
    leads.forEach((lead) => {
      const relevantDate = lead.handoverDate
        ? new Date(lead.handoverDate)
        : new Date(lead.createdAt);
      const month = relevantDate.getMonth() + 1; // JS months are 0-indexed
    ...
    revenues.forEach((rev) => {
      ...
      // BUG FIX: this used to derive monthNum/yearNum from rev.createdAt (when
      // the row was inserted in the DB), not from the row's own month/year —
      // that misfiled rows into the wrong financial year whenever a row was
      // created/backfilled at a different time than the period it represents.
    ...
    Object.keys(monthWiseRevenue).forEach((key) => {
      ...
      monthWiseRevenue[key] = {
        ...monthWiseRevenue[key],
        ...t,
        totalProfit: revenue - t.totalVendorExp - t.totalGST - t.totalExpenses,
      };
    });
...
// Helper function to convert month name to month number (inverse of getMonthName)
