  ...
  try {
    const leads = await db.lead.findMany({
      ...
      select: {
        ...
        store: true,
        createdAt: true,
        handoverDate: true, // ✅ NEW: needed to bucket by actual handover date
      },
    });
    ...
    leads.forEach((lead) => {
      // ❌ OLD (bug): bucketed by createdAt (when the lead was first
      // entered), not by when it actually closed — for a CLOSED lead these
      // can be months apart, so GST / Vendor Exp / Cash figures landed in
      // the wrong month/year/financial-year.
      // const date      = new Date(lead.createdAt);
      // ✅ FIX: every lead here is already status:"CLOSED", so bucket by
      // handoverDate; fall back to createdAt only if it's ever missing.
      const date = lead.handoverDate
        ? new Date(lead.handoverDate)
    ...
    // Grand totals
    const grandTotalGST = leads.reduce(
      (s, l) => s + (Number(l.totalGST) || 0),
      0,
    );
    const grandTotalVendorExp = leads.reduce(
      (s, l) => s + (Number(l.totalExp) || 0),
      0,
    );
    const grandReceiveCash = leads.reduce(
      (s, l) => s + (Number(l.receiveCash) || 0),
      0,
    );
    const grandReceiveOnline = leads.reduce(
      (s, l) => s + (Number(l.receiveOnline) || 0),
      0,
    );
    ...
    return NextResponse.json(
      {
        ...
        totalReceiveCash: grandReceiveCash,
        totalReceiveOnline: grandReceiveOnline,
        monthWise: Object.entries(monthWise).map(([month, d]) => ({
          month,
          ...d,
        })),
        yearWise: Object.entries(yearWise).map(([year, d]) => ({ year, ...d })),
        finYearWise: Object.entries(finYearWise).map(([finYear, d]) => ({
          finYear,
          ...d,
        })),
        storeWise: Object.entries(storeWise).map(([store, d]) => ({
          store,
          ...d,
        })),
      },
